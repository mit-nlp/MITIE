...
{
    const std::vector<std::string>& words = sentences[idx];
...
void train_id(const command_line_parser& parser)
{
    ...
    svm_multiclass_linear_trainer<sparse_linear_kernel<sample_type> > trainer;
    ...
    */
    ...
    multiclass_linear_decision_function<sparse_linear_kernel<sample_type> > df;
    ...
    ofstream fout("ner_chunk_id_classifier.dat", ios::binary);
    serialize(df, fout);
}
...
{
    multiclass_linear_decision_function<sparse_linear_kernel<sample_type> > df;
    ifstream fin("ner_chunk_id_classifier.dat", ios::binary);
    deserialize(df, fin);
    ...
    fin.close();
    fin.open("trained_segmenter.dat", ios::binary);
    total_word_feature_extractor fe;
    sequence_segmenter<ner_feature_extractor> segmenter;
    deserialize(fe, fin);
    deserialize(segmenter, fin);

    // find out how many in and out vocab words there are in the testing data
    std::vector<std::string> temp = fe.get_words_in_dictionary();
    std::set<std::string> vocab(temp.begin(), temp.end());
    double oov = 0, iv = 0;
    for (unsigned long i = 0; i < sentences.size(); ++i)
    {
        for (unsigned long j = 0; j < sentences[i].size(); ++j)
        {
            if (vocab.count(sentences[i][j]) != 0)
                ++iv;
            else
                ++oov;
        }
